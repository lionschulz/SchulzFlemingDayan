...
model.2o <- function(sI,   # sigma_I    (standard deviation of actor signal)
  ...
  # status print
  status_message <- paste("2nd-order model with: sigma_I =", round(sI,3),
                          ", tau_I =", round(tI,3),
                          ", and rho = ", rho,
                          ", cost = ", rS,
                          ", aF = ", aF)
  ...
  d <- 1
  SI <- computeCov(sI,tI,rho) # compute the covariance matrix (see eq. 8)
  ...
  # Avoid underflow through the tolearnce
  ...
  cI_2o_ap <- cI.2o(x,sI,tI,rho,1)     # confidence for positive values of xi
  cI_2o_am <- cI.2o(x,sI,tI,rho,-1)    # ... and for negative values
  ...
  # (see eqs. 31 - 36)
  cue_combination_list <- cue_combination.2o(sI,tI,rho) 
  ...
  # First, likelihood matrix of other source (d = -1)
  ...
  if (rS == 0){ # if seeking is free
    ...
    if (is.na(threshold_YI[2])){ 
      ...
      cI_cutoffs <- c(NA,cI.2o(threshold_YI[1], sI,tI,rho,1)) # set up only one cI
      
    } else {
      # if we get two thresholds
      interval_YI_search <- threshold_YI
      cI_cutoffs <- cI.2o(interval_YI_search, sI,tI,rho,1) 
  ...
  if (output == "av_search") {
    ...
    if (! av_search %in% c(0,1)) { 
        out$av_search_when_correct <- p_search_plus
        out$av_search_when_incorrect <- p_search_minus
    } else {
      out$av_search_when_correct <- av_search
      out$av_search_when_incorrect <- av_search
    }
...
# Qdifftol = .001
# denstol = 10e-300
# results <- model.2o(sI,tI,rho,sF,rS,output = "all")
